...
*                    vTaskLED 任务       ：LED闪烁
*                    vTaskMsgPro 任务    ：使用函数xEventGroupWaitBits接收任务vTaskTaskUserIF发送的事件标志
*                    vTaskStart 任务     ：按键扫描
...
*                 5. K2键按下，直接发送事件标志给任务vTaskMsgPro，设置bit0。
*                 6. K3键按下，直接发送事件标志给任务vTaskMsgPro，设置bit1。
*                 7. 任务vTaskMsgPro只有接收到bit0和bit1都被设置了才执行串口打印消息。
*              注意事项：
...
*********************************************************************************************************
*	函 数 名: vTaskMsgPro
*	功能说明: 使用函数xEventGroupWaitBits接收任务vTaskTaskUserIF发送的事件标志
*	形    参: pvParameters 是在创建该任务时传递的形参
*	返 回 值: 无
*   优 先 级: 3
*********************************************************************************************************
