...
*	文件名称 : main.c
*	版    本 : V1.0
*	说    明 : 本实验主要学习FreeRTOS的单队列消息覆盖（中断方式）
*              实验目的：
*                1. 学习FreeRTOS的单队列消息覆盖（中断方式）
...
*                    vTaskLED 任务       ：使用函数xQueueReceive接收定时器中断发送的消息队列数据(xQueue2)
*                    vTaskMsgPro 任务    ：使用函数xQueueReceive接收定时器中断发送的消息队列数据(xQueue1)
*                    vTaskStart 任务     ：按键扫描
...
*********************************************************************************************************
*	函 数 名: vTaskLED
*	功能说明: 使用函数xQueueReceive接收定时器中断发送的消息队列数据(xQueue2)
*	形    参: pvParameters 是在创建该任务时传递的形参
*	返 回 值: 无
*   优 先 级: 2
*********************************************************************************************************
...
*********************************************************************************************************
*	函 数 名: vTaskMsgPro
*	功能说明: 使用函数xQueueReceive接收定时器中断发送的消息队列数据(xQueue1)
*	形    参: pvParameters 是在创建该任务时传递的形参
*	返 回 值: 无
*   优 先 级: 3
*********************************************************************************************************
...
static void TIM_CallBack1(void)
{
	...
	/* 向消息队列发数据 */
	xQueueOverwriteFromISR(xQueue1,
				           (void *)&g_uiCount,
...
static void TIM_CallBack2(void)
{
	...
	/* 向消息队列发数据 */
	 xQueueOverwriteFromISR(xQueue2,
				            (void *)&ptMsg,
				            &xHigherPriorityTaskWoken);
