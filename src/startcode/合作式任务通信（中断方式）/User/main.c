...
*	文件名称 : main.c
*	版    本 : V1.0
*	说    明 : 本实验主要学习FreeRTOS的合作式任务通讯(中断方式)
*              实验目的：
*                1. 学习FreeRTOS的合作式任务通讯（中断方式）
*              实验内容：
...
*                 5. K3键按下，启动单次定时器中断，50ms后在定时器中断给合作式任务vCoRoutineLED发送消息
*                 6. 合作式任务调度，合作式任务之间不能相互抢占，但是可以被其它高优先级的任务抢占。
*                 7. 如果一个应用中同时含有合作式任务和其它类型任务，那些需要将合作式调度函数放在空
*                    闲任务的钩子函数里面。
*
*              注意事项：
    ...
    while(1)
    {
		...
		if (ucKeyCode != KEY_NONE)
		{
			switch (ucKeyCode)
			{
				...
				/* K3键按下，启动单次定时器中断，50ms后在定时器中断给合作式任务vCoRoutineLED发送消息 */
				case KEY_DOWN_K3:
					printf("K3键按下，启动单次定时器中断，50ms后在定时器中断给合作式任务vCoRoutineLED发送消息\r\n");
...
static void TIM_CallBack1(void)
{
	...
	/* 向合作式任务发送消息 */
	xCRWokenByPost = crQUEUE_SEND_FROM_ISR(xCoRoutineQueue,
										   &g_ucCount,
										   xCRWokenByPost);
}
