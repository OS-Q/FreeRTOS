...
*	文件名称 : main.c
*	版    本 : V1.0
*	说    明 : 本实验主要学习FreeRTOS的任务通知实现二值信号量（中断方式）
*              实验目的：
*                1. 学习FreeRTOS的任务通知实现二值信号量（中断方式）
*              实验内容：
...
*                    vTaskLED 任务       ：LED闪烁
*                    vTaskMsgPro 任务    ：使用函数ulTaskNotifyTake接收定时器中断发送的消息
*                    vTaskStart 任务     ：按键扫描
...
*                 5. 本实验二值信号量用于实现任务的同步。
*                 6. K2键按下，启动单次定时器中断，50ms后在定时器中断给任务vTaskMsgPro发消息
*
*              注意事项：
...
static void vTaskMsgPro(void *pvParameters)
{
	...
	const TickType_t xMaxBlockTime = pdMS_TO_TICKS(300); /* 设置最大等待时间300ms */
	
    while(1)
    {
		ulNotificationValue = ulTaskNotifyTake( pdTRUE,  /* 此参数设置为pdTRUE，接收到的notification value清零 */
                                               xMaxBlockTime ); /* 设置最大等时间 */
